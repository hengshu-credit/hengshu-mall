...
class MerchantProducts
{
    public static function constrain($query)
    {
        ...
        return $query->where(function ($q) use ($allowed) {
            ...
            if (in_array(MerchantInstaller::platformId(),$allowed,true)) $q->whereOr(function ($legacy) { $legacy->where('seller_shop_id',0)->where('mer_id',0); });
        });
    }
    ...
    public static function prepare(array $data, int $productId): array
    {
        ...
        if (array_key_exists('seller_shop_id',$data) && $data['seller_shop_id']!==null) {
            if (is_bool($data['seller_shop_id']) || filter_var($data['seller_shop_id'],FILTER_VALIDATE_INT)===false || (int)$data['seller_shop_id']<1) throw new AdminException('请选择有效的所属商户');
            $owner=(int)$data['seller_shop_id'];
        } else $owner=$old?self::owner($old):MerchantInstaller::platformId();
        $merchant=(new MerchantServices())->rawShop($owner,true);
        $changing=$old && $owner!==self::owner($old);
        if ($changing && (!empty($old['merchant_lock']) || !empty($old['is_show']) || !empty($old['sales']) || Db::name('store_order_cart_info')->where('product_id',$productId)->count())) throw new AdminException('商品已上架或已有订单，不能直接更换商户，请复制到目标商户');
        if ($changing && (Db::name('store_seckill')->where('product_id',$productId)->count() || Db::name('store_bargain')->where('product_id',$productId)->count() || Db::name('store_combination')->where('product_id',$productId)->count())) throw new AdminException('商品已有活动引用，不能更换所属商户');
        if (!empty($data['is_show']) && !(new MerchantServices())->available($merchant)) throw new AdminException('所属商户未营业、未通过审核或证件已过期，商品只能保存为下架');
        if ((!$old || $changing) && $merchant['state']==='closed') throw new AdminException('不能关联已关闭商户');
    ...
    public static function owner(array $product): int
    {
        ...
        if (empty($product['mer_id'])) return MerchantInstaller::platformId();
    ...
    public static function summaries(array $products): array
    {
        MerchantInstaller::ensure();
        $ids=[];
        foreach ($products as $row) { if (!empty($row['seller_shop_id'])) $ids[]=(int)$row['seller_shop_id']; elseif (empty($row['mer_id'])) $ids[]=MerchantInstaller::platformId(); }
        ...
        foreach ($products as &$row) {
            $id=!empty($row['seller_shop_id'])?(int)$row['seller_shop_id']:(empty($row['mer_id'])?MerchantInstaller::platformId():0);
            $row['seller_shop_id']=$id;
            $row['merchant']=$map[$id] ?? ['id'=>$id,'name'=>'商户归属待处理','type_name'=>'','subject_name'=>''];
    ...
    public static function assertPurchasable(int $productId): array
    {
        ...
        try { $id=self::owner($product); $row=(new MerchantServices())->rawShop($id); }
        catch (AdminException $e) { throw new ApiException('商品所属商户暂不可用'); }
        if (!(new MerchantServices())->available($row)) throw new ApiException('商品所属商户已暂停营业或资料失效');
    ...
    public static function orderSnapshot(array &$cartInfo): array
    {
        ...
        foreach ($cartInfo as &$cart) {
            ...
            try { $owner=self::owner($product); $merchant=(new MerchantServices())->rawShop($owner,true); }
            catch (AdminException $e) { throw new ApiException('商品归属尚未处理，暂不能下单'); }
            if (!(new MerchantServices())->available($merchant)) throw new ApiException('部分商品所属商户暂停营业或资料失效');
