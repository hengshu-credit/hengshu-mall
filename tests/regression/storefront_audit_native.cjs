...
async function navigate(url,login=false){const c=await get('/__navigate?url='+encodeURIComponent(url)+'&login='+(login?'1':'0'));for(let i=0;i<60;i++){if((await get('/__reports')).some(r=>r.seq===c.seq&&r.value?.navigated)){await pause(500);return;}await pause(250);}throw Error('Navigation not acknowledged');}
