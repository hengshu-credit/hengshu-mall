...
$changedOwner=Db::transaction(function () use ($products,$id2) { return $products::prepare(['seller_shop_id'=>$id2,'merchant_version'=>0,'is_show'=>0],20); });
checkMerchant('previous publication no longer locks product ownership',$changedOwner['seller_shop_id']===$id2);
...
$s->setState($id2,$s->info($id2)['version'],'open',$actor,merchantKey('open-second-for-claim'));
$optional=Db::transaction(function () use ($products) { return $products::prepare(['is_show'=>1],0); });
Db::name('store_product')->insert(['id'=>21,'store_name'=>'未分配测试商品','mer_id'=>0]+$optional);
checkMerchant('new product can publish without choosing a merchant',$optional['seller_shop_id']===0 && $products::assertPurchasable(21)['id']===0);
checkMerchant('unassigned products are visible and named honestly',in_array(21,$products::constrain(Db::name('store_product'))->column('id')) && $products::summaries([Db::name('store_product')->find(21)])[0]['merchant_name']==='未分配');
$unassignedCart=[['product_id'=>21,'cart_num'=>1]];
$unassignedSnapshot=Db::transaction(function () use ($products,&$unassignedCart) { return $products::orderSnapshot($unassignedCart); });
$products::assertOrderPayable($unassignedSnapshot+['paid'=>0]);
checkMerchant('unassigned orders keep an explicit original ownership snapshot',$unassignedSnapshot['seller_shop_id']===0 && (int)$unassignedCart[0]['merchant']['id']===0);
$claimed=$s->assignProducts([21],$id2,'claim',$actor,merchantKey('claim-optional'));
checkMerchant('merchant can claim a published unassigned product',$claimed['changed']===1 && Db::name('store_product')->where('id',21)->value('seller_shop_id')==$id2);
$s->assignProducts([21],$id,'assign',$actor,merchantKey('assign-after-claim'));
checkMerchant('later product assignment overrides merchant claim',Db::name('store_product')->where('id',21)->value('seller_shop_id')==$id);
$s->assignProducts([21],$id2,'claim',$actor,merchantKey('claim-optional'));
checkMerchant('retrying an earlier claim does not undo a later assignment',Db::name('store_product')->where('id',21)->value('seller_shop_id')==$id);
$s->assignProducts([21],$id2,'claim',$actor,merchantKey('claim-after-assign'));
checkMerchant('later claim overrides product assignment with equal priority',Db::name('store_product')->where('id',21)->value('seller_shop_id')==$id2);
$assignmentHistory=$s->histories($id,['event_type'=>'product_assignment'],true);
checkMerchant('both old and new merchants can trace product ownership changes',$assignmentHistory['count']===2 && $assignmentHistory['list'][0]['changes'][0]['before']===$id && $assignmentHistory['list'][0]['changes'][0]['after']===$id2 && $assignmentHistory['list'][0]['source']==='merchant_claim');
rejectMerchant('old product edit cannot overwrite a more recent allocation',function () use ($products,$id) { Db::transaction(function () use ($products,$id) { $products::prepare(['seller_shop_id'=>$id,'merchant_version'=>0,'is_show'=>1],21); }); });
rejectMerchant('same allocation request key cannot change targets',function () use ($s,$id,$actor) { $s->assignProducts([21],$id,'claim',$actor,merchantKey('claim-optional')); });
Db::name('store_product')->where('id',20)->update(['sales'=>3]);
$s->assignProducts([20],$id2,'claim',$actor,merchantKey('claim-existing-sales'));
checkMerchant('published products with sales can be reassigned without altering past orders',Db::name('store_product')->where('id',20)->value('seller_shop_id')==$id2 && json_decode($snapshot['merchant_snapshot'],true)[0]['id']==$id);
$newCart=[['product_id'=>20,'cart_num'=>1]];
$newSnapshot=Db::transaction(function () use ($products,&$newCart) { return $products::orderSnapshot($newCart); });
checkMerchant('subsequent orders use the latest product owner',$newSnapshot['seller_shop_id']===$id2);
$s->assignProducts([20],$id,'assign',$actor,merchantKey('restore-first-owner'));
$s->assignProducts([21],0,'assign',$actor,merchantKey('unassign-product'));
checkMerchant('allocation can remove ownership without taking a product offline',Db::name('store_product')->where('id',21)->value('is_show')==1 && Db::name('store_product')->where('id',21)->value('seller_shop_id')==0);
checkMerchant('candidate search filters unassigned products by ID',$s->productCandidates(['keyword'=>'21','owner'=>'0'])['count']===1);
rejectMerchant('batch assignment rejects missing products atomically',function () use ($s,$id,$actor) { $s->assignProducts([21,999999],$id,'assign',$actor,merchantKey('invalid-batch')); });
checkMerchant('failed batch leaves ownership unchanged',Db::name('store_product')->where('id',21)->value('seller_shop_id')==0);
Db::name('store_product')->insert(['id'=>22,'store_name'=>'回收站商品','mer_id'=>0,'is_del'=>1,'is_show'=>0]);
rejectMerchant('a later invalid batch item rolls back earlier ownership writes',function () use ($s,$id,$actor) { $s->assignProducts([21,22],$id,'assign',$actor,merchantKey('rollback-batch')); });
checkMerchant('late batch failure preserves the earlier product owner',Db::name('store_product')->where('id',21)->value('seller_shop_id')==0);
foreach ([[true],[21,'bad'],[],array_fill(0,201,21)] as $invalidIds) rejectMerchant('invalid product selection rejected',function () use ($s,$invalidIds,$id,$actor) { $s->assignProducts($invalidIds,$id,'assign',$actor,merchantKey('invalid-ids')); });
$s->setState($id,$s->info($id)['version'],'paused',$actor,merchantKey('pause-for-products'));
rejectMerchant('published products cannot be claimed by an unavailable merchant',function () use ($s,$id,$actor) { $s->assignProducts([21],$id,'claim',$actor,merchantKey('claim-paused')); });
$products::assertOrderPayable($unassignedSnapshot+['paid'=>0]);
...
checkMerchant('list access alone cannot assign or claim products',!$probe->can('product-assign'));
$roles->auth=['post'=>['merchant/product/assign']];
checkMerchant('one explicit permission enables both allocation entry points',$probe->can('product-assign') && !$probe->can('save'));
...
checkMerchant('reinstall preserves intentionally unassigned products',Db::name('store_product')->where('id',21)->value('seller_shop_id')==0);
