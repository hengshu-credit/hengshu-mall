...
(async () => {
  for (const method of ['submit', 'loginMobile']) {
    ...
    await instance[method]();
    assert.equal(instance.protocolConfirmVisible, true, 'Repeated clicks keep the one agreement dialog open');
    assert.equal(modals.length, 0, 'The styled agreement must not use platform-dependent native button order');
    assert.equal(requests.length, 0, 'No login request before consent');
    instance.finishLoginProtocol(false);
    assert.equal(instance.protocol, false);
    assert.equal(requests.length, 0, 'Declining must not log in');
    await instance[method]();
    instance.finishLoginProtocol(true);
    instance.finishLoginProtocol(true);
    await flush();
    ...
    await instance[method]();
    await flush();
    assert.equal(instance.protocolConfirmVisible, false, 'Retry must not ask for consent again');
    assert.equal(requests.length, 2);
    instance.ChangeIsDefault();
    await instance[method]();
    assert.equal(instance.protocolConfirmVisible, true, 'Manually unchecking must require consent again');
    instance.finishLoginProtocol(false);
  }
  ...
  const instance = page();
  instance.confirmLoginProtocol(() => assert.fail('Dismissing the dialog must not log in'));
  assert.equal(instance.dismissAppOverlay(), true, 'Android history dismisses the dialog before leaving the page');
  assert.equal(instance.protocolConfirmVisible, false);
  assert.equal(instance.protocol, false);
  instance.confirmLoginProtocol(() => assert.fail('Leaving the page must discard pending login'));
  options.onUnload.call(instance);
  instance.finishLoginProtocol(true);
  assert.equal(instance.protocol, false);
  assert.equal(instance._protocolLogin, null);
  console.log('PASS login agreement: confirm once, decline/back/unload, duplicate clicks, failed-login retry, third-party continuation');
