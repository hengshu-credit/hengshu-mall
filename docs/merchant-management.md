...
商品基础信息的“所属商户”为选填，不自动选择平台商户；未选择时显示“未分配”，可以正常上架、购买和下单。商品保存、草稿、复制与采集沿用同一字段；旧编辑请求不传字段时保留既有归属，清空选择则取消归属。

商品列表“更多 → 分配商户”和顶部批量分配，以及商户列表的“认领商品”，都通过同一接口更新归属，按最后一次成功提交的顺序生效。支持覆盖其他商户的归属、认领已上架或已有成交的商品；上架商品的目标商户须营业且资料有效。认领支持名称／ID搜索、未分配筛选和跨页选择，每次最多 200 件。两处共用“分配与认领商品”权限，商品归属调整及前后商户记录在相关商户的历史中。

归属修改按商品加锁、批次整体提交；失败整批回滚，旧请求重试不会覆盖之后的新操作。商品编辑携带归属版本，防止旧编辑页覆盖最新分配。已有订单和明细保留成交时的商户摘要，后续订单使用最新归属。已分配商品在目录、购买、下单和支付前继续检查商户经营状态。
...
命令可重复执行，补齐结构和菜单，保留已有类型名称、商户资料及平台默认商户。使用当前连接的表前缀。首次安装映射原 `mer_id=0` 且尚无新归属的旧平台商品／订单；后续更新保留有意未分配的商品和订单。原非零商户未映射的数据保持待处理，不统一归为自营。菜单权限需在角色中按需授予。
